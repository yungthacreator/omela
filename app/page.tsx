...
import { ArrowRight, Mic, ChevronLeft, ChevronRight, Search, FileCheck, Languages, PhoneOff, CalendarCheck, Stethoscope, Building2, Code2, Users } from "lucide-react";
...
const convos=[
  {lang:"English",flag:"\u{1F1EC}\u{1F1E7}",msgs:[
    ...
    {f:"l",t:"Found 3 practices near M1. The closest is City Health Centre, 0.3 miles."},
    {f:"a",t:"City Health Centre \u00b7 0.3mi \u00b7 Accepting patients"},
    {f:"l",t:"I have requested a registration callback for you. Bring photo ID and proof of address. Once registered, I will draft your inhaler request."},
    {f:"a",t:"Registration callback requested \u00b7 Response in 1 to 2 days"},
  ]},
  {lang:"Fran\u00e7ais",flag:"\u{1F1EB}\u{1F1F7}",msgs:[
    {f:"u",t:"J'ai besoin de renouveler mon ordonnance. Amlodipine 5mg."},
    {f:"l",t:"Je vais preparer une demande de renouvellement. Combien de jours de stock vous reste-t-il?"},
    {f:"u",t:"Environ 3 jours."},
    {f:"l",t:"Demande envoyee a votre cabinet. Je vous rappellerai de verifier dans 48 heures."},
    {f:"a",t:"Demande de renouvellement soumise \u00b7 Rappel dans 48h"},
  ]},
  {lang:"Yor\u00f9b\u00e1",flag:"\u{1F1F3}\u{1F1EC}",msgs:[
    ...
    {f:"a",t:"Spring Street Surgery \u00b7 0.4 maili \u00b7 Gba awon alaisan titun"},
    {f:"l",t:"Se o fe ki n pe won fun o lati gba ipade?"},
    {f:"u",t:"Beeni, e seun."},
    {f:"a",t:"Ti a ti beere ipe-pada \u00b7 Idahun laarin ojo 1"},
  ]},
];
function PhoneMockup(){
  const [li,setLi]=useState(0);const [vi,setVi]=useState(0);const [typing,setTyping]=useState(false);const cv=convos[li];
  useEffect(()=>{if(vi>=cv.msgs.length){const t=setTimeout(()=>{setLi(p=>(p+1)%convos.length);setVi(0);},2400);return()=>clearTimeout(t);}const msg=cv.msgs[vi];if(msg.f==="u"){setVi(p=>p+1);}else{setTyping(true);const t=setTimeout(()=>{setTyping(false);setVi(p=>p+1);},500+Math.random()*400);return()=>clearTimeout(t);}},[vi,li,cv.msgs]);
  return(
    <div className="phW"><motion.div className="phGlow" animate={{opacity:[0.12,0.3,0.12]}} transition={{duration:5,repeat:Infinity}}/>
      <motion.div className="phF" initial={{opacity:0,y:16}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.8}}>
        <div className="phB"><div className="phDI"/>
          <div className="phS">
            <div className="phH"><div className="phAv"><Image src="/laura-avatar.png" alt="Laura" fill sizes="24px" style={{objectFit:"cover"}}/></div><div><div className="phN">Laura <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M12 1L14.9 3.6L18.6 3.2L19.4 6.8L22.5 8.7L21.2 12.2L22.5 15.7L19.4 17.6L18.6 21.2L14.9 20.8L12 23.4L9.1 20.8L5.4 21.2L4.6 17.6L1.5 15.7L2.8 12.2L1.5 8.7L4.6 6.8L5.4 3.2L9.1 3.6L12 1Z" fill="#22C55E"/><path d="M8.5 12.5L10.5 14.5L15.5 9.5" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></div><div className="phOn"><span className="phOnD"/>online</div></div></div>
...
/* ═══ GLOBE ═══ */
const globeStories=[
  {country:"United Kingdom",flag:"\u{1F1EC}\u{1F1E7}",name:"Sarah, 34, Leeds",quote:"I called at 8:01am. Busy. By 8:15 all slots were gone. Three weeks of this.",laura:"Laura submits a structured callback request to your GP overnight. You wake up with a confirmed slot.",cx:48,cy:28},
  {country:"United States",flag:"\u{1F1FA}\u{1F1F8}",name:"Marcus, 28, Chicago",quote:"I delayed seeing a doctor for six months because I was terrified of the cost.",laura:"Laura surfaces lower-cost options, community clinics, and sliding-scale providers near you automatically.",cx:28,cy:35},
  {country:"Nigeria",flag:"\u{1F1F3}\u{1F1EC}",name:"Amina, 41, Lagos",quote:"Four doctors for ten thousand people. I spent two days trying to find someone who could see me.",laura:"Laura searches availability across providers in real time and routes you to whoever can see you soonest.",cx:50,cy:52},
  {country:"India",flag:"\u{1F1EE}\u{1F1F3}",name:"Priya, 55, Jaipur",quote:"My mother lives two hours from the nearest clinic. She never knows when to worry.",laura:"Laura runs an urgency assessment remotely and tells you whether to travel now or monitor safely at home.",cx:68,cy:42},
  {country:"Brazil",flag:"\u{1F1E7}\u{1F1F7}",name:"Lucas, 23, London (from Sao Paulo)",quote:"I moved to London and could not explain my symptoms in English. The receptionist was kind but we could not understand each other.",laura:"Laura captures symptoms in your language, generates a bilingual clinical note, and sends it to the practice before you arrive.",cx:32,cy:58},
];
function GlobeSection(){
  const [a,setA]=useState(0);const tr=useRef<ReturnType<typeof setTimeout>|null>(null);
  const reset=useCallback(()=>{if(tr.current)clearTimeout(tr.current);tr.current=setTimeout(()=>setA(p=>(p+1)%globeStories.length),6000);},[]);
    ...
    <section className="globeSec"><div className="container">
      ...
      <div className="globeW">
        <div className="globeV"><svg viewBox="0 0 100 100" className="globeSvg">
          <circle cx="50" cy="50" r="47" fill="none" stroke="#C8BFA8" strokeWidth="0.6"/>
          <circle cx="50" cy="50" r="47" fill="url(#gG)"/>
          {[20,30,40,50,60,70,80].map(y=><ellipse key={y} cx="50" cy="50" rx={Math.sqrt(Math.max(0,2209-(y-50)*(y-50)))} ry="3.5" fill="none" stroke="#D0C9B8" strokeWidth="0.2" opacity="0.45" transform={`translate(0,${y-50})`}/>)}
          {[-30,-15,0,15,30].map(x=><ellipse key={x} cx="50" cy="50" rx="3.5" ry="47" fill="none" stroke="#D0C9B8" strokeWidth="0.2" opacity="0.35" transform={`rotate(${x},50,50)`}/>)}
          <path d="M44,22 L48,20 L52,22 L53,26 L50,30 L46,28 Z" fill="#D4CDB8" opacity="0.4"/>
          <path d="M46,30 L52,30 L54,38 L50,42 L44,38 L43,34 Z" fill="#D4CDB8" opacity="0.35"/>
          <path d="M20,30 L35,28 L38,34 L32,40 L22,38 L18,34 Z" fill="#D4CDB8" opacity="0.3"/>
          <path d="M28,50 L34,46 L36,54 L32,62 L26,58 Z" fill="#D4CDB8" opacity="0.3"/>
          <path d="M62,34 L74,32 L78,40 L72,48 L64,44 L60,38 Z" fill="#D4CDB8" opacity="0.3"/>
          {globeStories.map((st,i)=>{const nx=globeStories[(i+1)%globeStories.length];return<motion.path key={`a${i}`} d={`M${st.cx},${st.cy} Q50,${Math.min(st.cy,nx.cy)-12} ${nx.cx},${nx.cy}`} fill="none" stroke={c.warm} strokeWidth={i===a?"0.8":"0.2"} opacity={i===a?0.65:0.1} strokeDasharray="3 2" animate={i===a?{strokeDashoffset:[0,-5]}:{}} transition={{duration:1.2,repeat:Infinity,ease:"linear"}}/>})}
          {[0,1].map(p=><motion.circle key={`p${p}`} r="0.9" fill={c.warm} opacity="0.6" animate={{cx:[globeStories[a].cx,50,globeStories[(a+1)%globeStories.length].cx],cy:[globeStories[a].cy,Math.min(globeStories[a].cy,globeStories[(a+1)%globeStories.length].cy)-12,globeStories[(a+1)%globeStories.length].cy]}} transition={{duration:2.5,delay:p*1.2,repeat:Infinity,ease:"linear"}}/>)}
          {globeStories.map((st,i)=>(<g key={st.country}>
            {i===a&&<motion.circle cx={st.cx} cy={st.cy} r="4" fill={c.warm} opacity="0.15" animate={{r:[4,8,4],opacity:[0.15,0.28,0.15]}} transition={{duration:2,repeat:Infinity}}/>}
            <circle cx={st.cx} cy={st.cy} r={i===a?3:1.5} fill={i===a?c.warm:"#B0A894"} style={{cursor:"pointer",transition:"all 0.4s"}} onClick={()=>setA(i)}/>
          </g>))}
          <defs><radialGradient id="gG" cx="38%" cy="33%"><stop offset="0%" stopColor="#F8F6F1" stopOpacity="0.02"/><stop offset="100%" stopColor="#E5DFD2" stopOpacity="0.1"/></radialGradient></defs>
        </svg></div>
    ...
    <div className="wrap">
      ...
      <section className="heroSec"><div className="container">
        <div className="heroTxt">
          ...
          <FI delay={0.1}><p className="heroSub">Laura is an AI agent that finds doctors, submits prescription requests, explains referral letters, and prepares bilingual clinical notes. She does not just answer questions. She gets things done.</p></FI>
      ...
      <section id="how" className="sec"><div className="container">
        <FI><div className="shW"><h2 className="serif shT">Laura does not just answer health questions.<br/>She acts on them.</h2></div></FI>
        <div className="capGrid">{[
          {icon:<Search size={22}/>,title:"Finds doctors and submits requests",body:"Laura searches practices near you, checks availability, and submits registration or callback requests on your behalf. You do not make the call."},
          {icon:<FileCheck size={22}/>,title:"Drafts prescriptions and tracks refills",body:"Laura prepares your repeat prescription request, submits it to your practice portal, and sets reminders before you run out. No more chasing."},
          {icon:<Languages size={22}/>,title:"Prepares bilingual clinical notes",body:"Laura captures your symptoms in your language, translates them into a structured clinical note, and sends it to the receptionist before you arrive."},
      ...
      <section className="sec" style={{paddingTop:0}}><div className="container">
        ...
        <FI><div className="baGrid">
          <div className="baCard baBf"><h3 className="baTi">Without Laura</h3>{["You call the GP. Busy signal. Try again tomorrow.","You email the pharmacy. No reply for three days.","You get a referral full of medical jargon. No idea what to do.","You arrive in a new city with zero medical contacts.","You try to explain symptoms in a language that is not yours."].map(item=><div key={item} className="baIt"><span className="baX">&#10007;</span>{item}</div>)}</div>
      ...
      <section className="sec"><div className="container">
        ...
        <div className="audGrid">{[
          {icon:<Users size={24}/>,title:"People",desc:"Laura acts on your behalf. She finds your next GP, drafts your prescription request, explains your hospital letter, and submits your callback. You do not lift the phone.",color:c.accent},
          {icon:<Building2 size={24}/>,title:"Care teams",desc:"Laura intercepts and structures incoming patient demand before it hits reception. She triages by urgency, captures symptoms in structured format, and routes callbacks intelligently. Your team handles patients, not admin.",color:c.green},
          {icon:<Code2 size={24}/>,title:"Developers",desc:"Embed Laura's care-navigation engine into your product. Provider search, urgency assessment, prescription routing, and multilingual intake available via API and embeddable components.",color:c.warm,soon:true},
        ].map((item,i)=>(<FI key={item.title} delay={i*0.07}><div className="audCard"><motion.div className="audIc" style={{background:`${item.color}12`,color:item.color}} animate={{rotate:[0,3,-3,0]}} transition={{duration:4+i,repeat:Infinity,ease:"easeInOut"}}>{item.icon}</motion.div><div style={{display:"flex",alignItems:"center",gap:"6px"}}><h3 className="audTi">{item.title}</h3>{item.soon&&<Soon/>}</div><p className="audBd">{item.desc}</p></div></FI>))}</div>
      </div></section>

      {/* LANGUAGE */}
      <section className="sec" style={{paddingTop:0}}><div className="container"><FI><div className="glC"><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",flexWrap:"wrap"}}><h2 className="serif glT">Laura speaks your language.</h2><Soon/></div><p className="glB">40+ languages. Laura responds in the language you start with, captures symptoms accurately, and generates bilingual notes for the practice.</p><div className="glF">{["\u{1F1EC}\u{1F1E7} UK","\u{1F1FA}\u{1F1F8} US","\u{1F1EB}\u{1F1F7} France","\u{1F1F3}\u{1F1EC} Nigeria","\u{1F1EE}\u{1F1F3} India","\u{1F30D} 40+ more"].map(f=><span key={f} className="glFi">{f}</span>)}</div></div></FI></div></section>
      ...
      <section className="secDk"><div className="container">
        ...
        <div className="g2" style={{marginTop:"28px"}}><FI><SDKTerminal/></FI><FI delay={0.08}><div className="dkC"><h3 className="dkCT">Infrastructure for health apps</h3><p className="dkCB">Add Laura to patient portals, telehealth platforms, or internal tooling. She handles the care-access layer so your team focuses on the product.</p><div className="ftL" style={{marginTop:"14px"}}>{["Provider search and availability API","Urgency assessment engine","Prescription and callback routing","Multilingual intake with bilingual notes"].map(f=><div key={f} className="ftR" style={{color:"rgba(255,255,255,0.7)"}}><Chk dark/><span>{f}</span></div>)}</div></div></FI></div>
      </div></section>

      {/* PRICING */}
      <section id="pricing" className="sec"><div className="container">
        <FI><div className="shW"><h2 className="serif shT">Simple access. No surprises.</h2></div></FI>
        <div className="prGrid">{[
          {name:"People",price:"Free",period:"during early access",desc:"Full access to Laura for care navigation, prescription requests, and provider search.",cta:"Join waitlist",hl:false},
          {name:"Care Teams",price:"Starting at \u00A32,500",period:"/mo",desc:"Laura handling intake, urgency triage, and structured callbacks for your practice.",cta:"Request demo",hl:true,badge:"Best for practices"},
          {name:"Developer",price:"Custom",period:"",desc:"API access to Laura's care-navigation engine for your product.",cta:"Talk to us",hl:false,soon:true},
          ...
          <div className="prPr"><span className="serif prAmt">{p.price}</span>{p.period&&<span className="prPd">{p.period}</span>}</div>
      ...
      <section id="waitlist" className="sec"><div className="container"><FI><div className="wlC">
        <div className="shW"><h2 className="serif shT">Join the waitlist.</h2><p className="shB">We are onboarding in batches. Tell us who you are.</p></div>
      ...
      <footer className="ft"><div className="container ftIn">
        ...
        <div className="ftBtm"><p>&copy; 2026 Omela. Laura is a care navigation agent, not a medical professional. For emergencies, call 999 or 911.</p></div>
      </div></footer>
    </div>
  </>);
}
...
.soon{display:inline-flex;padding:3px 8px;border-radius:6px;background:linear-gradient(135deg,rgba(37,99,235,0.08),rgba(37,99,235,0.04));color:${c.accent};font-size:10px;font-weight:700;letter-spacing:0.02em;border:1px solid rgba(37,99,235,0.12);white-space:nowrap}
...
.phBb{max-width:88%;padding:5px 7px;font-size:8px;line-height:1.38;word-break:break-word}.phBbU{background:#E7FFDB;color:#111;border-radius:8px 8px 2px 8px}.phBbL{background:#fff;color:#111;border-radius:8px 8px 8px 2px;box-shadow:0 1px 1px rgba(0,0,0,0.03)}.phMt{display:flex;justify-content:flex-end;margin-top:1px}
...
/* Capability cards */
.capGrid{display:grid;grid-template-columns:1fr;gap:12px;margin-top:28px}
.capCard{padding:24px;border:1px solid ${c.border};border-radius:20px;background:rgba(255,255,255,0.92);transition:all 0.25s}.capCard:hover{box-shadow:0 12px 32px rgba(0,0,0,0.05);transform:translateY(-2px)}
.capIc{width:48px;height:48px;border-radius:14px;display:flex;align-items:center;justify-content:center;background:${c.accentSoft};color:${c.accent};border:1px solid rgba(37,99,235,0.08);margin-bottom:12px}
.capTi{font-size:17px;font-weight:800;letter-spacing:-0.03em}.capBd{font-size:13px;line-height:1.68;color:${c.sub};margin-top:4px}
...
/* Audience cards */
.audGrid{display:grid;grid-template-columns:1fr;gap:12px;margin-top:28px}
.audCard{padding:24px;border:1px solid ${c.border};border-radius:20px;background:rgba(255,255,255,0.92);transition:all 0.25s}.audCard:hover{box-shadow:0 8px 24px rgba(0,0,0,0.04)}
.audIc{width:48px;height:48px;border-radius:14px;display:flex;align-items:center;justify-content:center;margin-bottom:12px;border:1px solid rgba(0,0,0,0.04)}
.audTi{font-size:18px;font-weight:800;letter-spacing:-0.03em}.audBd{font-size:13px;line-height:1.72;color:${c.sub};margin-top:6px}
...
/* Pricing */
.prGrid{display:grid;grid-template-columns:1fr;gap:12px;margin-top:28px}
.prCard{background:rgba(255,255,255,0.92);border:1px solid ${c.border};border-radius:20px;padding:24px;position:relative;transition:all 0.25s}.prCard:hover{box-shadow:0 8px 24px rgba(0,0,0,0.04)}
.prCardDk{background:linear-gradient(180deg,#111318,#0D0F14)!important;border:1px solid ${c.borderDk}!important;color:#fff!important;box-shadow:0 14px 36px rgba(0,0,0,0.18)!important}
.prBg{position:absolute;top:12px;right:12px;border-radius:6px;padding:3px 8px;background:rgba(255,255,255,0.1);color:rgba(255,255,255,0.6);font-size:9px;font-weight:700}
.prNm{font-size:15px;font-weight:800}.prPr{margin-top:8px;display:flex;align-items:baseline;gap:4px}.prAmt{font-size:clamp(28px,5vw,40px);letter-spacing:-0.05em}.prPd{font-size:13px;color:${c.muted}}.prCardDk .prPd{color:rgba(255,255,255,0.4)}.prDs{margin-top:6px;font-size:13px;line-height:1.68;color:${c.sub}}.prCardDk .prDs{color:rgba(255,255,255,0.5)}
.prBt{margin-top:16px;width:100%}.prBtW{background:#fff!important;color:${c.dark}!important}
...
.ft{background:${c.dark};padding:32px 0 20px;color:#fff}.ftIn{}.ftTop{display:flex;flex-direction:column;gap:20px}.ftBr{display:flex;align-items:center;gap:7px;text-decoration:none;color:#fff}.ftBrN{font-size:13px;font-weight:800}.ftBrS{font-size:8px;font-weight:800;letter-spacing:0.1em;color:${c.accent};margin-top:1px}.ftFd{font-size:11px;color:rgba(255,255,255,0.25);margin-top:6px;font-style:italic}.ftCols{display:flex;gap:32px}.ftCol{display:flex;flex-direction:column;gap:6px}.ftColT{font-size:10px;font-weight:800;color:rgba(255,255,255,0.3);text-transform:uppercase;letter-spacing:0.1em;margin-bottom:2px}.ftLk{font-size:12px;color:rgba(255,255,255,0.5);font-weight:500;transition:color 0.2s}.ftLk:hover{color:#fff}.ftBtm{border-top:1px solid rgba(255,255,255,0.06);padding-top:14px;margin-top:20px;font-size:10px;color:rgba(255,255,255,0.2)}
@media(min-width:640px){.container{padding:0 24px}.sec{padding:72px 0}.secDk{padding:72px 0}.navR{height:64px}.navLks{display:flex}.navCt{padding:10px 18px!important;font-size:13px!important}.heroBt{flex-direction:row}.heroBt .btnP,.heroBt .btnS{width:auto}.capGrid{grid-template-columns:repeat(3,1fr)}.baGrid{grid-template-columns:repeat(2,1fr)}.audGrid{grid-template-columns:repeat(3,1fr)}.prGrid{grid-template-columns:repeat(3,1fr)}.g2{grid-template-columns:repeat(2,1fr);gap:20px}.wlF{grid-template-columns:1.2fr 0.8fr auto}.wlBt{width:auto}.wlC{padding:28px}.phW{max-width:240px}.ftTop{flex-direction:row;justify-content:space-between}.trmB{height:300px}.globeW{flex-direction:row;align-items:center;gap:36px}.globeV{max-width:320px;flex-shrink:0}.globeC{text-align:left}.globeCtry{justify-content:flex-start}.globeQ{text-align:left}.globeNm{text-align:left}}
