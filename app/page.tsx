...
/* ── Status Bar with animated ticker ── */
function StatusBar() {
      ...
      <div className="container statusInner">
        ...
        <div className="statusTicker">
          <motion.div className="statusTickerTrack" animate={{ x: ["0%", "-50%"] }} transition={{ duration: 20, ease: "linear", repeat: Infinity }}>
            {items.map((s, i) => (
              <span key={`${s}-${i}`} className="statusSvc">
                <span className="statusSvcDot" />{s}
              </span>
            ))}
          </motion.div>
        </div>
...
export default function Page() {
  ...
  const plans = [
    ...
    { name: "Developer", price: "Custom", period: "", desc: "For teams planning to embed Laura into their products.", ft: ["API access (roadmap)", "Embeddable components", "Usage analytics", "Technical onboarding"], hl: false, cta: "Talk to us" },
  ];
      ...
      <div className="wrap">
        ...
        <section className="heroSec">
          <div className="container">
            ...
            {/* Phone — full width on mobile, positioned right on desktop */}
            <div className="heroPhone">
              <PhoneMockup />
            </div>

            {/* Capability cards — ALWAYS full width, below hero content */}
            <FadeIn delay={0.24}>
              <HeroCaps />
            </FadeIn>
            ...
            <div className="g3" style={{ marginTop: "48px" }}>{plans.map((p, i) => (
              <FadeIn key={p.name} delay={i * 0.1}><div className={`card ${p.hl ? "cardDk" : ""}`} style={{ position: "relative" }}>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: "4px", marginTop: "16px", flexWrap: "wrap" }}>
                  <span className="serif" style={{ fontSize: "clamp(32px, 5vw, 44px)", letterSpacing: "-0.05em" }}>{p.price}</span>
...
/* ══════════════════════════════════════════════════════════════
   CSS — TRUE MOBILE FIRST
   Base = mobile. Tablet = 640px. Desktop = 960px. Wide = 1100px.
   ══════════════════════════════════════════════════════════════ */
const CSS = `
...
.navCtaL{display:none}
.navCtaS{display:inline}
...
/* Phone — full width on mobile, centered */
...
/* ── Capability Cards — FULL WIDTH, below hero ── */
...
.termBody{padding:14px;min-height:220px;overflow-x:auto}
...
@media(min-width:640px){
  ...
  .pBody{min-height:300px;max-height:300px}
...
@media(min-width:960px){
  ...
  /* Hero becomes side-by-side grid via CSS Grid on the container */
  ...
  .pBody{min-height:340px;max-height:340px}
